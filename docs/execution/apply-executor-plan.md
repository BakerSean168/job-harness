...
### 18.17 Lease-scoped user-browser extension bridge

The second BrowserBackend axis is now implemented without importing the old `job-application-copilot` extension's ledger/profile ownership. `ExtensionBrowserBackend` implements the same `BrowserBackendPort` / `BrowserDriverPort` used by Steel, while a new Manifest V3 companion under `integrations/browser-extension/` owns only a user-controlled Chrome tab and narrow DOM primitives. It has **no Job/Application ledger, Resume Profile model, scoring engine, outreach policy or SubmissionIntent state**. The legacy extension was used as behavioral reference only; the new bridge and page driver are clean typed re-expressions around the frozen R019 contracts.

The relay is intentionally in-memory. Driver commands/results, applicant field values and optional PDF bytes are never persisted to Job Harness SQLite by the bridge. The extension uses optional host permissions granted by the user, does not request cookies/debugger/webRequestBlocking, and does not read the Job Harness global bearer. Upload and screenshot capabilities remain opt-in and are disabled by default in Extension Options.

Browser-command authority is split three ways. The global/user bearer may create a five-minute one-time pairing code. The extension exchanges that code directly for an HMAC-signed Agent Token bound to one `agentId`; the long-lived signing key remains only on Oracle2 and a consumed pairing code cannot be reused. The extension Agent Token can only register/poll/report its bridge channel and cannot read Career/Resume APIs or issue SubmitAuthorization. The Apply Worker still uses only `JOB_HARNESS_EXECUTOR_AUTH_TOKEN`.

Most importantly, the worker bearer alone is not enough to drive user Chrome. Every `/invoke` request carries the current `ExecutionAttempt` lease scope (`attemptId`, `executorId`, raw lease token); the server verifies the lease against SQLite before queueing any browser command. Form writes require frozen `allowFormFill=true`, Resume upload also requires the exact frozen Resume Artifact, and initial navigation must match the frozen Listing URL. Browser `click` is further restricted to the currently characterized Nowcoder pre-submit entry action: the Attempt must still be before the external-effect boundary, have `allowApplicationEntry=true`, be bound to `nowcoder-ats`, and carry the exact expected text `立即申请`. The page driver rechecks that visible text immediately before clicking. Final submit is therefore still impossible through this generic extension bridge.

A synthetic end-to-end integration test now runs the **normal ExecutionAttempt protocol** through an ExtensionBrowserBackend and a simulated user-Chrome agent: Job Harness creates a Job + planned SubmissionIntent, the worker claims the Attempt, the extension-backed driver exposes a real application FormIR, lease-scoped native applicant facts fill name/email, and the same Attempt reaches `waiting_for_user` with a ReviewSnapshot while `externalEffectState=not_crossed`, the SubmissionIntent remains `planned`, and Applications remain zero. Separate Chromium coverage injects the actual MV3 `page-driver.js` into a live page and verifies control/action scanning, native value writes, select handling, PDF `DataTransfer` upload and form-hash drift.

The Settings workspace now owns the pairing UX instead of asking the user to copy a durable token. It shows the configured public bridge URL and can generate a short-lived one-time pairing code; Extension Options consumes it, stores only the scoped Agent Token in Chrome extension local storage, and can explicitly unpair. This is the correct long-term replacement for sharing the old extension's server token/configuration.

Repository validation at this slice is **75 test files / 154 tests** through `pnpm check`, including the Browser Extension contract gate, boundary guard, ChatGPT **32-tool / zero external-side-effect** gate, OpenAPI drift check, TypeScript and Web production build. This completes the code-level user-browser backend proof required by R019, but not the real-browser production proof: no actual Windows Chrome extension is paired yet, user-browser Resume upload remains disabled by default, and no live recruiting-site final submit has been authorized or executed.

### 18.18 Health-aware multi-backend dispatch

Apply Worker registration and claiming are now browser-health-aware. A worker checks its configured BrowserBackend before registration/claim, registers or heartbeats `degraded` when that backend is unavailable, and **does not claim an Attempt while degraded**. This matters for the user-browser backend: the extension worker can stay safely online before Chrome is paired without stealing queued work and then failing it merely because the browser agent is absent. Steel receives the same fail-closed behavior.

The Web safe-fill dispatcher is no longer hard-coded to Steel. It derives compatible ready backends from live ExecutorRegistration capabilities and requires `fill_only + humanControl + persistentSession + resumeUpload` for the current frozen-PDF workflow. If multiple compatible backends exist, the user can select the execution browser; the user-Chrome `extension` backend is preferred in the selector when it is genuinely ready. The Server Action independently revalidates the chosen backend against current ready executors before creating the Attempt, so a stale page cannot dispatch into an offline or under-capable browser. Duplicate active Attempt detection and immutable Revision/PDF validation remain unchanged.

This lets Oracle2 stage a second extension-backed worker in `degraded` state before the real browser is connected. Promotion of that worker to compatible `ready` status still requires the paired extension to be online **and** explicit Resume-upload capability to be enabled on both the extension and worker. Final submit remains unavailable.

### 18.19 Isolated real-Chrome validation scope

The first production-paired Windows Chrome proof no longer needs a fake Job, Listing, SubmissionIntent or Application. A separate in-memory `BrowserExtensionValidationRegistry` now owns short-lived browser-health canaries. The global/user bearer may create a validation run only for the configured Job Harness synthetic ATS origin and the exact `/labs/apply-canary?run=...` route; worker and extension Agent tokens cannot create one. This keeps validation evidence outside the Career and Apply business aggregates, so production counters and immutable application history are not polluted by test entities.

Validation runs deliberately expose a smaller command language than a real ExecutionAttempt. They allow isolated `session_acquire`, read/scan/hash operations and basic text/select/checkbox writes, but reject `click`, `upload`, `screenshot` and arbitrary navigation. The acquired Chrome tab is bound to one returned `sessionRef`. Because the browser remains user-owned, the server re-reads the tab URL immediately before every later validation command and rejects the run if the tab has drifted away from the frozen synthetic URL. A canary can therefore prove the real extension transport and DOM driver without becoming a second path around the SubmissionIntent / ReviewSnapshot / SubmitAuthorization safety boundary.

The matching Web route is a noindex, no-store, CSP-restricted synthetic ATS surface. It exposes normal name/email/school/major controls plus a visa field that should remain manual. Its synthetic submit endpoint stores only field-presence/length evidence and a submit counter, never applicant values. A successful fill-only real-browser canary must leave `submitCount=0`; any accidental submit is therefore independently observable. This validation scope is an operational health/proof mechanism only and cannot authorize a recruiting-site external effect.

The validation scope also has one deliberately narrow file-transfer probe: `upload` is accepted only for a `.pdf` / `application/pdf` payload targeting the synthetic page's exact `#resume` input. The canary page computes and displays local filename/byte-count/SHA-256 evidence without uploading the file to its server. This permits an end-to-end check of the real Chrome `DataTransfer` path with the exact immutable Job Harness Resume Artifact while still denying screenshot, click, arbitrary file controls and all external hosts.

The first exact-PDF real-Chrome probe also found and closed a transport bug before promotion: the MCP SDK's default Express JSON parser caps request bodies at roughly 100 KiB, while the extension upload contract can legitimately carry a base64 PDF several hundred KiB in size. Job Harness now uses the ordinary conservative JSON limit everywhere except the two browser-command invoke routes. Those routes pre-authenticate the global/worker bearer before attaching a 22 MiB parser (matching the contract's 20,000,000-character base64 ceiling). A regression test sends a 750 KiB upload envelope and proves it reaches lease authorization instead of failing with HTTP 413. This exception does not enlarge unauthenticated pairing or general Career/MCP request bodies.

### 18.20 First-class Applicant Profile and Application AnswerSet

The browser executor no longer needs Resume to carry every reusable application fact. R019 now introduces two deliberately separate first-class aggregates: **Applicant Profile** owns base identity/contact/education and general application preferences, while **Application AnswerSet** owns explicit recurring ATS answers that should not be embedded in a role-specific Resume. Both are exposed in `/settings`, use optimistic concurrency, and publish immutable content-hashed revisions on semantic changes. SQLite schema **v11** stores mutable heads and append-only revisions separately.

Migration remains strangler-style rather than copy/paste. On first v11 startup, Job Harness seeds Applicant Profile once from the existing first-class Resume Domain and creates an empty default AnswerSet. The browser extension never owns either model. Resume remains responsible for role-specific document wording (positioning, work/project bullets, skills and artifacts); Applicant Profile becomes authoritative for overlapping autofill base keys such as name/contact/education. The old compatibility profile bundle is not reintroduced.

Dispatch now freezes the latest Applicant Profile Revision and AnswerSet Revision into `ApplyBundle` with their SHA-256 content hashes, alongside the existing immutable Resume Revision/PDF evidence. The lease-scoped ApplicantData grant resolves only from those frozen revisions. Its value-free catalog merges Resume-specific facts, Applicant Profile base facts and explicit AnswerSet literals into one deterministic `applicant-snapshot:<sha256>` version. Editing Settings after dispatch cannot alter an in-flight form fill.

AnswerSet entries may be scoped to a recruiting-site hostname. `legal` and `protected` entries require explicit aliases/literal values and advertise `allowAiMapping=false`; an absent answer remains unresolved and therefore manual rather than being inferred by a model. This preserves the earlier R019 rule that AI may help map ordinary form semantics but must not invent work-authorization, visa, EEO or similar answers.

The REST/OpenAPI/client surface now exposes the mutable Applicant Profile and AnswerSet to the trusted Job Harness Web control plane. Worker access remains narrower: it can obtain only a value-free catalog and explicitly requested literal values under a current ExecutionAttempt lease. ChatGPT MCP remains unchanged at **32 tools / 0 external recruiting-site side-effect tools**.

Repository validation for the v11 slice is **79 test files / 160 tests**, plus TypeScript, package-boundary guards, Browser Extension contract checks, generated OpenAPI drift validation, the ChatGPT 32-tool static gate and the Next production build. R019 remains open because this completes applicant-data ownership and freeze semantics, not a live recruiting-site final-submit proof.

### 18.21 Applicant-data v11 production proof

Oracle2 was backed up immediately before the migration while production was still schema v10; the backup independently reported `integrity_check=ok` with **115 Jobs / 41 Applications / 41 ApplicationSubmissions**. The `c7ab9aa` runtime was then deployed and migrated the same database to **SQLite v11**. Post-migration `integrity_check` remained `ok`, `foreign_key_check` returned no violations, and Career/Apply counters stayed **115 Jobs / 41 Applications / 41 ApplicationSubmissions / 0 SubmissionIntents / 0 ExecutionAttempts**.

The one-time bootstrap created exactly **1 Applicant Profile + revision 1** and **1 default empty Application AnswerSet + revision 1**. Redacted API verification confirms the profile contains name, phone, email, GitHub and one education record derived from the first-class Resume Domain; missing optional facts remain visibly unset rather than guessed. `/settings` is now the trusted place to edit those base facts and recurring application answers.

Both production executors returned to `ready` after restart: Steel and the paired Windows Chrome extension backend each advertise `fill_only`, human control, persistent sessions and Resume upload; external submit remains disabled. The actual paired Chrome agent is online. Authenticated production MCP smoke remains **32 tools**, with **115 known Jobs / 41 Applications / 0 SubmissionIntents / 5 Resume Profiles**, so this migration does not broaden the ChatGPT external-side-effect boundary.
