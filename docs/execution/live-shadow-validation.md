...
## Policy-gated application entry canary

The Nowcoder observed adapter now separates **application entry** from both form filling and final submission. A frozen Attempt policy must contain `allowApplicationEntry=true` before the adapter may click anything. Even then, it accepts exactly one enabled visible action whose normalized text is exactly `立即申请`; ambiguous or missing actions fail closed. Other observed adapters keep `enter=false`.

A fresh ephemeral real-site canary ran this path through the normal Job Harness lease/worker protocol. It performed exactly **one pre-submit navigation action**, transitioned from `job_detail` to `login_required`, then stopped with the SubmissionIntent still `planned`, `externalEffectState=not_crossed`, zero ReviewSnapshots, zero Applications, zero applicant-data access, and **zero irreversible submit actions**. The retained anonymous browser was released after verification. This does not authorize login, OTP handling, form submission, or any final recruiting-site side effect.
