...
- [x] **JH-R018:** converged the remaining recurring-automation central Job Ledger through a read-only, idempotent application-port importer: 5 historical DiscoveryRuns / 21 source Jobs became 115 canonical Jobs total, 41 Applications stayed unchanged, legacy aliases were attached on strong evidence, and lossy DeepSeek/Tencent company-only rows were reconciled without placeholder Applications.
- [ ] **JH-R019:** expose the existing browser automation as an Apply Executor module backed by SubmissionIntent, immutable Resume Artifact evidence and explicit executor capability/status; retire duplicate ledger/resume ownership after end-to-end proof.
