...
R016 control-plane activation has now advanced one step further: a dedicated Job Harness Secure MCP Tunnel exists in the target ChatGPT workspace scope and the isolated Oracle2 `job-harness-chatgpt-tunnel.service` is enabled. It uses the separately installed `tunnel-client` v0.0.14 runtime, binds only `127.0.0.1:20901/mcp`, keeps both control-plane and downstream bearer credentials in a private `0600` env file, initializes a real `job-harness` v0.2.0 MCP session, reports `/healthz = live` and `/readyz = ready`, and has a verified successful OpenAI control-plane poll. The unrelated broad `oracle2-tunnel.service` remains active and unchanged.

R016 is now complete. The ChatGPT workspace custom MCP app was created against the dedicated Tunnel and its tool scan surfaced **32 actions**, matching the production MCP catalog and the repository static gate. The visible catalog includes the expected Career read surface such as `career_context_get`, `career_jobs_search`, `career_job_get`, `career_job_duplicate_check`, `career_applications_list`, `career_application_get`, `career_campaigns_list`, `career_campaign_get`, `career_resumes_list`, and `career_pipeline_stats`; the full catalog remains guarded by the same **18 required workflow tools / 0 external-side-effect tools** invariant. The app is therefore connected end to end: ChatGPT -> OpenAI Secure MCP Tunnel -> isolated Oracle2 tunnel-client -> authenticated Job Harness MCP -> application ports -> SQLite/Resume runtime. Real recruiting-site submission still remains outside MCP and must use the separate executor plus `SubmissionIntent` protocol.
