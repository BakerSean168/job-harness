...
R016 remains intentionally open only for the ChatGPT workspace action: create/refresh the custom MCP app with **Connection: Tunnel**, select the dedicated Job Harness tunnel, scan the catalog, and approve/enable it. The repository static gate still expects **32 tools**, **18 required workflow tools**, and **0 external-side-effect tools** before write testing.
