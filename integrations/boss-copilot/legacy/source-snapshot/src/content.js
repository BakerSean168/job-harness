(() => {
  ...
  function getRepeatGroupLabelText(element) {
    ...
    const labels = controls
      ...
      .filter((label) => label && !isOptionOnlyLabel(label) && !/(OpenJobAutofill|Job Application Copilot)/.test(label));
  ...
  function collectResumeUploadInputs() {
  ...
  function isResumeUploadInput(input) {
    const container = findFieldContainer(input);
    const context = normalizeText([
      getLabelByFor(input),
      getWrappingLabel(input),
      getAriaLabelText(input),
      getAdapterLabelText(input),
    ...
    const positive = /简历|个人简历|附件简历|上传简历|resume|curriculum\s*vitae|\bcv\b/i.test(context);
    const negative = /证件照|身份证|证件|头像|照片|生活照|成绩单|学位证|毕业证|证书|作品集|portfolio|签名|体检|银行卡/i.test(context);
    if (!positive || negative) {
      return false;
    }

    const accept = String(input.getAttribute("accept") || "").toLowerCase();
    if (accept && /image\//.test(accept) && !/pdf|application\/pdf|\.pdf/.test(accept)) {
      return false;
    }
    return true;
  }

  async function attachActiveResumeFile() {
    const inputs = collectResumeUploadInputs();
    if (inputs.length === 0) {
      return {
        attached: 0,
        found: 0,
        message: "当前页面没有识别到明确的简历 PDF 上传框。证件照、证件、作品集等附件不会自动处理。"
      };
    }

    const resumeFile = await sendRuntimeMessage({ type: "JAC_GET_ACTIVE_RESUME_FILE" });
    if (!resumeFile?.base64) {
      throw new Error("没有拿到当前简历 PDF 数据。");
    }

    const bytes = base64ToBytes(resumeFile.base64);
    let attached = 0;
    const failures = [];

    for (const input of inputs) {
      try {
        const file = new File([bytes], resumeFile.fileName || "resume.pdf", {
        ...
        if (!input.files || input.files.length === 0) {
          throw new Error("页面拒绝接收程序设置的附件");
        }

        const markTarget = getResumeUploadMarkTarget(input);
        markElement(markTarget, "filled", `已准备简历附件：${resumeFile.fileName || "resume.pdf"}`);
        attached += 1;
      } catch (error) {
        const markTarget = getResumeUploadMarkTarget(input);
        markElement(markTarget, "uncertain", "简历附件需要手动选择");
    ...
    return {
      ...
      profileId: resumeFile.profileId || "",
      profileLabel: resumeFile.profileLabel || "",
      source: resumeFile.source || "",
      message: attached > 0
        ? `已把 ${resumeFile.fileName || "当前简历"} 准备到 ${attached} 个简历上传框；提交前请人工确认附件名称和页面预览。`
        : `识别到 ${inputs.length} 个简历上传框，但页面拒绝程序设置附件，请手动选择 ${resumeFile.fileName || "当前简历 PDF"}。`
    };
  }

  function getResumeUploadMarkTarget(input) {
    const container = findFieldContainer(input);
  ...
  function ensureProfilePanel() {
    ...
    title.textContent = "Job Application Copilot";
    ...
    for (const candidate of candidates) {
      ...
      for (const part of parts) {
        ...
        if (
          ...
          !/请输入|请选择|上传文件|内容列表|搜索分类或内容|OpenJobAutofill|Job Application Copilot/.test(cleaned)
        ) {
  ...
  function getSemanticBucket(text, category = "") {
    ...
    if (textKey === "学历" || /学历层次|教育经历学历/.test(key)) {
      return "educationLevel";
    }
    if (textKey === "学位" || /学位类型|教育经历学位/.test(key)) {
      return "degree";
    }
    if (/绩效考核|考核年度|考核等级/.test(key)) {
    ...
    if (textKey === "姓名" || /真实姓名/.test(key)) {
      return "fullName";
    }
    if (textKey === "姓" || /中文姓|姓氏/.test(key)) {
      return "lastName";
    }
    if (textKey === "名" || /中文名|名字/.test(key)) {
      return "firstName";
    }
    ...
    if (textKey === "公司" || /工作单位|单位名称|公司名称|实习单位/.test(key)) {
      return "employer";
    }
    if (/部门/.test(key)) {
      return "department";
    ...
    if (textKey === "生源地" || /生源所在地|生源户口/.test(key)) {
      return "sourcePlace";
    }
    ...
    if (textKey === "六级" || /大学英语六级|cet6/.test(key)) {
      return "cet6Taken";
    }
    ...
    if (textKey === "四级" || /大学英语四级|cet4/.test(key)) {
      return "cet4Taken";
    }
    ...
    if (textKey === "toefl" || /托福/.test(key)) {
      return "toeflTaken";
    }
    if (textKey === "ielts" || /雅思/.test(key)) {
      return "ieltsTaken";
    }
    if (textKey === "gre") {
      return "greTaken";
  ...
  function isAttachmentLikeField(field, fieldLabel = "") {
    ...
    return field?.type === "file" || /上传|附件|照片|证件照|简历附件/.test(ownText);
  }
  ...
  function scoreAutofillCandidate(field, entry, fieldLabel, fieldCategory) {
    ...
    if (isAttachmentLikeField(field, fieldLabel)) {
      score = -999;
    }
  ...
  function createAutofillCandidate(field, entry, score) {
    ...
    const shouldAutoFill =
      ...
      value !== "" &&
      field.canFill &&
      !isAttachmentLikeField(field, fieldLabel);
  ...
  function renderProfilePanel() {
    ...
    if (collapseBtn) {
      ...
      collapseBtn.title = profilePanelCollapsed ? "展开 Job Application Copilot 资料面板" : "收起 Job Application Copilot 资料面板";
    }
    if (copyCategoryBtn) {
  ...
  function captureCurrentJobContext() {
    ...
    const firstText = (selectors, maxLength) => {
      for (const selector of selectors) {
        try {
          ...
          // Ignore selectors that are unsupported by a page implementation.
        }
      }
      return "";
    };

    const title = firstText(titleSelectors, 240) || normalizeText(document.title, 240);
    const company = firstText(companySelectors, 160);
    const detail = firstText(detailSelectors, 30000) || normalizeText(document.body?.innerText || "", 30000);
  ...
  async function handleContentMessage(message) {
    if (message.type === "JAC_CAPTURE_JOB_CONTEXT") {
      return captureCurrentJobContext();
    }

    if (message.type === "JAC_ATTACH_ACTIVE_RESUME") {
      return attachActiveResumeFile();
    }
