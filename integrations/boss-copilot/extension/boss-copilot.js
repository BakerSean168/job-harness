...
// @connect      oracle.taile92a8e.ts.net
// @connect      127.0.0.1
// @connect      localhost
// ==/UserScript==
...
(function () {
    ...
    function jacHttpRequest(details) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Job Harness compatibility runtime can execute this mature userscript
            // directly as a Chromium content script without Tampermonkey. The
            // BOSS bridge explicitly allows the zhipin.com origin, so a normal
            // CORS fetch preserves the old request/response contract.
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), Math.max(1000, Number(details.timeout || 600000)));
            fetch(details.url, {
                ...
                resolve({ status: response.status, response: text });
            }).catch(reject).finally(() => clearTimeout(timeoutId));
        });
    }
