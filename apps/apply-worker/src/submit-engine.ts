import type { ExecutionAttempt } from '@job-harness/apply-contracts';
import type { BrowserDriverPort } from '@job-harness/apply-browser';
import type { ApplySiteAdapter, ApplySiteAdapterRegistry, ApplySiteSubmitResult } from '@job-harness/apply-adapters';

export interface SubmitExecutionEngineOptions {
  readonly siteAdapters: ApplySiteAdapterRegistry;
}

export interface SubmitPreflightResult {
  readonly adapterId: string;
  readonly adapterVersion: string;
  readonly liveUrl: string;
  readonly formStateHash: string;
}

export class SubmitExecutionEngine {
  private readonly siteAdapters: ApplySiteAdapterRegistry;

  constructor(options: SubmitExecutionEngineOptions) {
    this.siteAdapters = options.siteAdapters;
  }

  /**
   * Read-only eligibility check for the retained browser state. The worker runs
   * this before begin-submit so adapter drift or page drift cannot consume the
   * external-effect boundary. execute() repeats the same checks after the
   * boundary to close the TOCTOU window immediately before the site click.
   */
  async preflight(input: { readonly attempt: ExecutionAttempt; readonly browser: BrowserDriverPort }): Promise<SubmitPreflightResult> {
    const adapter = this.resolveSubmitAdapter(input.attempt);
    const liveUrl = await input.browser.refreshCurrentUrl();
    const liveProbe = adapter.probe({ url: liveUrl });
    if (!liveProbe.supported) {
      throw new Error(`Live browser page '${new URL(liveUrl).hostname}' no longer matches submit adapter '${adapter.descriptor.id}'`);
    }
    const formStateHash = (await input.browser.formStateHash()).toLowerCase();
    return {
      adapterId: adapter.descriptor.id,
      adapterVersion: adapter.descriptor.version,
      liveUrl,
      formStateHash,
    };
  }

  async execute(input: { readonly attempt: ExecutionAttempt; readonly browser: BrowserDriverPort; readonly expectedFormStateHash: string }): Promise<ApplySiteSubmitResult> {
    const beforeSubmit = await this.preflight({ attempt: input.attempt, browser: input.browser });
    if (beforeSubmit.formStateHash !== input.expectedFormStateHash.toLowerCase()) {
      throw new Error('Live form state changed after submit authorization; refusing to call the site submit adapter');
    }
    const adapter = this.resolveSubmitAdapter(input.attempt);
    return adapter.submit!(input.browser);
  }

  private resolveSubmitAdapter(attempt: ExecutionAttempt): ApplySiteAdapter {
    const targetUrl = attempt.bundle.listingUrl;
    if (!targetUrl) throw new Error('Frozen ApplyBundle has no Listing URL');
    const requested = attempt.adapterId ?? attempt.requiredAdapterId;
    const adapter = this.siteAdapters.resolve({
      url: targetUrl,
      semantics: 'formal_application',
      requiredAdapterId: requested,
    });
    if (!adapter) throw new Error(`No submit adapter supports '${new URL(targetUrl).hostname}'`);
    if (!adapter.descriptor.capabilities.submit || !adapter.submit) {
      throw new Error(`Site adapter '${adapter.descriptor.id}' has no submit capability`);
    }
    if (!attempt.adapterVersion || adapter.descriptor.version !== attempt.adapterVersion) {
      throw new Error(`Submit adapter '${adapter.descriptor.id}' runtime version '${adapter.descriptor.version}' does not match frozen Attempt version '${attempt.adapterVersion ?? 'none'}'`);
    }
    return adapter;
  }
}
