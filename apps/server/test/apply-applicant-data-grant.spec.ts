...
describe('lease-scoped applicant data derived from immutable Resume Revision', () => {
  it('exposes a value-free catalog and resolves only requested literal facts without giving the worker unrestricted Resume access', async () => {
    ...
    const savedApplicant = await request('/applicant-profile', 'global-secret', { method:'PUT', body:JSON.stringify({
      ...
      profile: { ...applicantContext.body.profile, gender:'male', birthDate:'2001-04-09', location:'杭州', jobSearchStatus:'actively_looking', careerIdentity:'student', education: applicantContext.body.profile.education.map((item: any, index: number) => index === 0 ? { ...item, admissionType:'unified' } : item) },
    }) });
    ...
    expect(catalog.body.entries).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ key: 'career.identity', valueType: 'choice', allowAiMapping: false, source: 'job-harness-applicant-profile' }),
      expect.objectContaining({ key: 'education[0].school', source: 'job-harness-applicant-profile' }),
      expect.objectContaining({ key: 'education[0].admission_type', valueType: 'choice', allowAiMapping: false, source: 'job-harness-applicant-profile' }),
    ...
    const resolved = await request(`/execution-attempts/${attemptId}/applicant-data/resolve`, 'worker-secret', {
      method: 'POST', body: JSON.stringify({ executorId: 'worker-applicant', leaseToken, keys: ['person.full_name','contact.email','person.gender','person.birth_date','contact.location','career.job_search_status','career.identity','education[0].school','education[0].admission_type','work[0].description','legal.visa_sponsorship_required','legal.work_authorization'] }),
    });
    ...
    expect(resolved.body.values).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ key: 'career.identity', value: 'student', literal: true }),
      expect.objectContaining({ key: 'education[0].school', value: '四川农业大学', literal: true }),
      expect.objectContaining({ key: 'education[0].admission_type', value: 'unified', literal: true }),
