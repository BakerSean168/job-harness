    ...
    <div className="workspace-page analytics-page">
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-two-column analytics-comparison-grid">
          ...
          <section className="dashboard-panel"><div className="dashboard-panel-heading"><h2>{copy.resumes.title}</h2></div>{snapshot.resumeUsage.length ? <div className="dashboard-compact-table-wrap"><table className="dashboard-compact-table"><thead><tr><th>{copy.resumes.resume}</th><th>{copy.resumes.applications}</th><th>{copy.resumes.submissions}</th><th>{copy.resumes.screening}</th><th>{copy.resumes.assessment}</th><th>{copy.resumes.interview}</th><th>{copy.resumes.offer}</th></tr></thead><tbody>{[...snapshot.resumeUsage].sort((a,b)=>b.applications-a.applications || b.submissions-a.submissions).map((item)=><tr key={item.resume.id}><td><Link className="text-link" href={scoped('/resumes',campaignId,{resume:item.resume.id})}>{item.resume.name}</Link></td><td>{item.applications}</td><td>{item.submissions}</td><td>{item.applicationsByStage.screening}</td><td>{item.applicationsByStage.assessment}</td><td>{item.applicationsByStage.interview}</td><td>{item.applicationsByStage.offer}</td></tr>)}</tbody></table></div>:<p className="dashboard-empty-copy">{copy.resumes.empty}</p>}<p className="dashboard-note">{copy.resumes.note}</p></section>
        </div>
      </div>
    </div>
  );
