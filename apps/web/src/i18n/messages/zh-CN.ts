...
export const zhCN: MessageCatalog = {
  ...
  settingsWorkspace: {
    ...
    applicant: { profileTitle: '求职者基础资料', profileDescription: '自动填表的基础事实真值。姓名、联系方式、性别、出生日期、当前城市、求职状态、求职身份和教育信息优先从这里读取；简历仍保留岗位定向内容。每次保存都会产生不可变 Revision，投递时冻结具体版本。', name: '姓名', phone: '手机', email: '邮箱', gender: '性别', genderMale: '男', genderFemale: '女', birthDate: '出生日期', location: '当前城市 / 所在地', jobSearchStatus: '当前求职状态', statusActivelyLooking: '正在积极求职', statusOpen: '对机会开放', statusNotLooking: '暂不求职', careerIdentity: '求职身份', careerIdentityStudent: '学生', careerIdentityNewGraduate: '应届 / 新毕业生', careerIdentityProfessional: '职场人', unknown: '未填写', github: 'GitHub', website: '个人网站', targetRoles: '目标岗位', targetCities: '目标城市', availableFrom: '可到岗时间', education: '教育经历', school: '学校', institutionTag: '院校标签', major: '专业', degree: '学历 / 学位', admissionType: '招生类型', admissionTypeUnified: '统招', admissionTypeNonUnified: '非统招', department: '学院 / 院系', educationLocation: '学校所在地', period: '就读时间', saveProfile: '保存基础资料', answersTitle: '投递问答库', answersDescription: '保存不适合放进简历、但会在 ATS 表单反复出现的明确答案。可按站点 Host 限定；没有配置的法律/敏感问题会停下来等待人工处理。', noAnswers: '还没有配置额外投递问答。', answerLabel: '问题名称', answerKey: '规范 Key', answerType: '值类型', sensitivity: '敏感级别', answerValue: '明确答案', aliases: '问题别名', siteHost: '限定站点（可选）', addAnswer: '+ 添加问答', removeAnswer: '删除此问答', saveAnswers: '保存问答库', sensitiveWarning: 'legal / protected 类型必须是你明确填写的 literal answer；Job Harness 不允许 AI 推断或生成这类答案。', saving: '保存中…', saved: '已保存' },
    applicantReference: { title: '资料参考 / 快速复制', description: '恢复旧 Copilot 的资料参考能力，但真值仍只保存在 Job Harness。可搜索、逐项复制或复制当前类别；不会把 Applicant 数据写回浏览器扩展存储。', search: '搜索资料', searchPlaceholder: '姓名、学校、岗位、常见问题…', copyValue: '复制', copyCategory: '复制本类', copyVisible: '复制当前结果', copied: '已复制', empty: '没有匹配的资料。', privacyNote: '这里位于已认证的 Job Harness Web 会话中。Browser Bridge 仍只持有浏览器控制权限，不会获得 Applicant / Resume API 的读取权限。', educationPrefix: '教育经历 ', siteScope: '限定站点', globalScope: '全站通用', categories: { profile: '基础资料', job: '求职偏好', education: '教育经历', answers: '投递问答' }, fields: { name: '姓名', phone: '手机', email: '邮箱', gender: '性别', birthDate: '出生日期', location: '当前城市 / 所在地', careerIdentity: '求职身份', jobSearchStatus: '当前求职状态', github: 'GitHub', website: '个人网站', notes: '备注', targetRoles: '目标岗位', targetCities: '目标城市', availableFrom: '可到岗时间', school: '学校', institutionTag: '院校标签', degree: '学历 / 学位', major: '专业', admissionType: '招生类型', department: '学院 / 院系', educationLocation: '学校所在地', period: '就读时间' }, values: { male: '男', female: '女', student: '学生', newGraduate: '应届 / 新毕业生', professional: '职场人', activelyLooking: '正在积极求职', open: '对机会开放', notLooking: '暂不求职', unified: '统招', nonUnified: '非统招' } },
    ...
    characterization: { title: 'ATS 只读 Characterization', description: '使用已登录的 Windows Chrome 在隔离标签页读取当前智联/猎聘岗位的按钮、表单结构和状态信号。不会填写、上传、点击或截图，也不会保存页面字段值/正文。', agent: '浏览器', targetUrl: '岗位 URL', pendingBindings: '待建立站点简历绑定', pendingDescription: '这些 planned Intent 已冻结简历、尚无历史执行且缺少精确站点简历绑定。先打开目标岗位并手动停在简历选择/最终确认层，再运行只读检查。', pendingNone: '当前没有需要建立站点简历绑定的新 planned Intent。', openTarget: '打开目标岗位', expectedResume: '预期 Job Harness 简历', targetHint: '当前仅允许智联 zhaopin.com/jobdetail/*.htm 和猎聘 liepin.com/job/<id>.shtml。', mode: '检查模式', isolatedMode: '隔离标签页（岗位详情）', stagedMode: '复用已打开岗位页（最终确认层）', stagedHint: '最终确认层模式不会新开、跳转或点击页面。请先在已登录 Chrome 中手动把同一岗位打开到简历选择/最终确认层，再运行检查；找不到完全匹配的已打开页面会直接失败。', run: '开始只读检查', running: '检查中…', error: '检查失败', result: '脱敏证据', signals: '状态信号', actions: '动作', controls: '控件', formHash: 'Form Hash', noSignals: '无', loginRequired: '当前招聘站仍要求登录。请先在这个已打开的岗位标签页完成登录，再手动进入简历选择 / 最终确认层，然后重新运行 staged 只读检查。', noActions: '未发现动作', noControls: '未发现控件', bindingTitle: '站点在线简历绑定', bindingDescription: '只有本次只读证据中真实观察到的简历标签才能绑定。来自待处理 Intent 时会精确绑定它已冻结的 immutable Revision/PDF；手工 URL 模式才回退到该 Profile 当前最新版本。后续 Profile 更新不会静默漂移旧绑定。', siteResumeLabel: '站点简历标签', profile: 'Job Harness 简历', confirmBinding: '确认绑定', bindingPending: '绑定中…', bindingSuccess: '绑定成功', bindingError: '绑定失败', noBindingCandidates: '本次页面没有观察到可绑定的在线简历选项；可能需要在隔离标签页由你手动进入简历选择层后重新扫描。', existingBindings: '当前有效绑定', noBindings: '暂无站点在线简历绑定。', revoke: '撤销绑定' },
  ...
  jobsWorkspace: {
    ...
    emailApplication: { back: '返回岗位', title: '邮件申请包', description: '收件人、正文和精确 PDF 已冻结；真正发送必须使用同一 draftHash，并在发送前跨越 durable boundary。', recipient: '收件人', subject: '主题', body: '正文', attachment: '附件', draftHash: 'Draft Hash', intentState: 'SubmissionIntent', providerBoundary: '发送邮件属于外部副作用。邮件包已冻结收件人、正文和精确 PDF；SMTP Worker 只有在当前 draftHash 获得短时授权后才会 claim 并发送。', downloadResume: '下载冻结简历 PDF', authorizeSend: '授权发送（5 分钟）', authorized: '已授权，等待 Email Worker；有效期至', workerNotConfigured: 'SMTP Email Worker 尚未配置凭据；你仍可让 ChatGPT 使用已连接的 Gmail 插件发送此邮件包，并通过 begin/confirm 闭环落账。' },
    filters: {
